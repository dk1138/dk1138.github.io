...
class FinancialEngine {
    constructor(data) {
        ...
        this.mode = data.mode;
        this.expenseMode = data.expenseMode;
        this.expensesByCategory = data.expensesByCategory;
        this.totalDebtInitial = data.totalDebt;
    ...
    calculateTaxDetailed(inc, prov, tDat) {
        ...
        const D = tDat, fC = this.calculateProgressiveTax(inc, D.FED.brackets, D.FED.rates), pC = this.calculateProgressiveTax(inc, D[prov]?.brackets||[999999999], D[prov]?.rates||[0.10]);
    ...
    calcInflows(yr, i, p1, p2, age1, age2, alive1, alive2, isRet1, isRet2, c, bInf) {
        let res = { p1: { gross:0, earned:0, cpp:0, oas:0, pension:0, windfallTaxable:0, windfallNonTax:0, postRet:0 }, p2: { gross:0, earned:0, cpp:0, oas:0, pension:0, windfallTaxable:0, windfallNonTax:0, postRet:0 } };
    ...
    handleSurplus(amount, p1, p2, alive1, alive2, i, tfsaLim, rrspLim1, rrspLim2) {
        let r = amount;
        this.strategies.accum.forEach(t => { 
            if(r<=0) return;
            if(t==='tfsa'){ 
                if(alive1 && (!this.inputs['skip_first_tfsa_p1']||i>0)){
                    let take = Math.min(r, tfsaLim); p1.tfsa+=take; r-=take;
                } 
                if(alive2 && r>0 && (!this.inputs['skip_first_tfsa_p2']||i>0)){
                    let take = Math.min(r, tfsaLim); p2.tfsa+=take; r-=take;
                } 
            }
            else if(t==='rrsp'){
                let priority = [];
                ...
                if (p1HasRoom && p2HasRoom) {
                    if (p1.rrsp < p2.rrsp) priority = [{p: p1, room: rrspLim1}, {p: p2, room: rrspLim2}];
                    else priority = [{p: p2, room: rrspLim2}, {p: p1, room: rrspLim1}];
                } else if (p1HasRoom) priority = [{p: p1, room: rrspLim1}];
                else if (p2HasRoom) priority = [{p: p2, room: rrspLim2}];

                priority.forEach(obj => {
                    if (r > 0) {
                        let take = Math.min(r, obj.room);
                        obj.p.rrsp += take; r -= take;
                    }
                });
            }
            else if(t==='nreg'){
                if(alive1){ p1.nreg+=r; p1.acb+=r; r=0; }
            } 
            else if(t==='cash'){
                if(alive1){ p1.cash+=r; r=0; }
            } 
            else if(t==='crypto'){
                if(alive1){ p1.crypto+=r; r=0; }
            }
        });
    }

    handleDeficit(amount, p1, p2, curInc1, curInc2, alive1, alive2, taxBrackets, onWithdrawal, age1, age2) {
        let df = amount;
        let runInc1 = curInc1;
        let runInc2 = curInc2;
        ...
        const wd = (p, t, a, pfx, mRate) => { 
            ...
            if (isTaxable && onWithdrawal) onWithdrawal(pfx, 0, tk);
    ...
    runSingleSimulation(simContext) {
        ...
        let totalDebt = this.totalDebtInitial;
        ...
        for (let i=0; i<=yearsToRun; i++) {
            ...
            const inflows = this.calcInflows(yr, i, person1, person2, age1, age2, alive1, alive2, isRet1, isRet2, consts, bInf);
            ...
            simProperties.forEach(p => {
                if (p.sellEnabled && p.sellAge === age1) {
                    ...
                    const netCash = proceeds - p.mortgage - (proceeds * 0.05);
            ...
            if (surplus > 0) {
                this.handleSurplus(surplus, person1, person2, alive1, alive2, i, consts.tfsaLimit*bInf, rrspRoom1, rrspRoom2);
            } else {
                ...
                for(let pass=0; pass<5; pass++) {
                    ...
                    let currentDeficit = totalOutflows - (dynNet1 + dynNet2 + addedToTaxableIncome);
                    if (currentDeficit < 1) break; 
                    
                    this.handleDeficit(currentDeficit, person1, person2, taxableIncome1, taxableIncome2, alive1, alive2, taxBrackets, (pfx, taxAmt, grossAmt) => {
            ...
            nwArray.push(liquidNW + (realEstateValue - realEstateDebt));
...
self.onmessage = function(e) {
    const data = e.data;
    const engine = new FinancialEngine(data);
    const trajectories = [];
    ...
    for (let i = 0; i < data.simulations; i++) {
        ...
        trajectories.push(engine.runSingleSimulation(simContext));
    }
