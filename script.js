...
class RetirementPlanner {
    ...
    generateProjectionTable(onlyCalcNW = false) {
        ...
        for (let i=0; i<=yearsToRun; i++) {
            ...
            // Pre-calculate base gross inflow (without withdrawals) for accurate cash flow loop
            // Base Gross = Employment + Pension + Benefits + Windfalls + Yields (Cash received)
            const baseGrossP1 = inflows.p1.gross + inflows.p1.benefits + inflows.p1.pension + inflows.p1.windfallTaxable + inflows.p1.windfallNonTax + (person1.nreg * person1.nreg_yield);
            const baseGrossP2 = inflows.p2.gross + inflows.p2.benefits + inflows.p2.pension + inflows.p2.windfallTaxable + inflows.p2.windfallNonTax + (alive2 ? (person2.nreg * person2.nreg_yield) : 0);
            const totalBaseGross = baseGrossP1 + baseGrossP2;
            ...
            if (surplus > 0) {
                ...
                for(let pass=0; pass<5; pass++) {
                    // Calculate Total Tax Liability based on current taxable income state
                    ...
                    // Get total gross withdrawals taken so far
                    let currentWithdrawals = Object.values(flowLog.withdrawals).reduce((a,b)=>a+b,0);

                    // Cash Flow Check:
                    // Cash In = Base Gross + Withdrawals
                    // Cash Out = Expenses + Debt + Tax
                    // Deficit = Cash Out - Cash In
                    
                    let totalCashIn = totalBaseGross + currentWithdrawals;
                    let totalCashOut = totalOutflows + dynTax1.totalTax + dynTax2.totalTax;
                    
                    let currentDeficit = totalCashOut - totalCashIn;
                    
                    if (currentDeficit < 5) break; // Close enough ($5 tolerance)
                    ...
                    this.handleDeficit(currentDeficit, person1, person2, taxableIncome1, taxableIncome2, alive1, alive2, flowLog, wdBreakdown, taxBrackets, (pfx, taxAmt, grossAmt) => {
                        // This callback updates the income tracking for the NEXT pass tax calc
            ...
            if(!onlyCalcNW) {
                ...
                const hhNetCash = grossInflow - (tax1.totalTax + tax2.totalTax); // Net cash flow excluding expenses
    ...
    handleDeficit(amount, p1, p2, curInc1, curInc2, alive1, alive2, log, breakdown, taxBrackets, onTaxIncrease, age1, age2) {
        ...
        const wd = (p, t, a, pfx, mRate) => { 
            ...
            if (isTaxable) {
                 if(onTaxIncrease) onTaxIncrease(pfx, 0, tk);
            } else if (t === 'nreg') {
                 // Calculate Capital Gain Portion
                 // Simplified: Gain = Withdrawal * (1 - (ACB / Value_Before_Withdrawal))
                 // Since p[t] is already reduced by tk, Value_Before = p[t] + tk
                 let valBefore = p[t] + tk;
                 let ratio = 0;
                 if (valBefore > 0) ratio = Math.max(0, (valBefore - p.acb) / valBefore);
                 
                 let gain = tk * ratio;
                 // Reduce ACB proportionally
                 p.acb -= (p.acb * (tk / valBefore)); 
                 
                 // Add 50% of gain to taxable income
                 if(onTaxIncrease) onTaxIncrease(pfx, 0, gain * 0.5);
            }
