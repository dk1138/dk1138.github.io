...
class RetirementPlanner {
    ...
    init() {
        const setup = () => {
            ...
            if (savedData) {
                ...
                catch(e) { console.error("Failed load", e); this.renderDefaults(); }
    ...
    updateThemeIcon(theme) {
        ...
        btn.className = theme === 'dark' ? 'btn btn-sm btn-outline-secondary' : 'btn btn-sm btn-outline-dark';
    }
    ...
    exportToCSV() {
        ...
        const h = ["Year", "P1 Age", mode==="Couple"?"P2 Age":null, "P1 Base Income", mode==="Couple"?"P2 Base Income":null, "P1 Post-Ret Inc", mode==="Couple"?"P2 Post-Ret Inc":null, "P1 Benefits", mode==="Couple"?"P2 Benefits":null, "P1 DB Pension", mode==="Couple"?"P2 DB Pension":null, "Windfall", "P1 Taxes", mode==="Couple"?"P2 Taxes":null, "Total Expenses", "Mortgage Payment", "Debt Payment", "Surplus/Deficit", "P1 TFSA", "P1 RRSP", "P1 Non-Reg", "P1 Cash", "P1 Crypto", mode==="Couple"?"P2 TFSA":null, mode==="Couple"?"P2 RRSP":null, mode==="Couple"?"P2 Non-Reg":null, mode==="Couple"?"P2 Cash":null, mode==="Couple"?"P2 Crypto":null, "Liquid Net Worth", "Home Equity", "Total Net Worth"].filter(x=>x);
        const rows = d.map(r => [r.year, r.p1Age, mode==="Couple"?(r.p2Age||""):null, Math.round(r.incomeP1), mode==="Couple"?Math.round(r.incomeP2):null, Math.round(r.postRetP1||0), mode==="Couple"?Math.round(r.postRetP2||0):null, Math.round(r.benefitsP1), mode==="Couple"?Math.round(r.benefitsP2):null, Math.round(r.dbP1), mode==="Couple"?Math.round(r.dbP2):null, Math.round(r.windfall), Math.round(r.taxP1), mode==="Couple"?Math.round(r.taxP2):null, Math.round(r.expenses), Math.round(r.mortgagePay), Math.round(r.debtPay), Math.round(r.surplus), Math.round(r.assetsP1.tfsa), Math.round(r.assetsP1.rrsp), Math.round(r.assetsP1.nreg), Math.round(r.assetsP1.cash), Math.round(r.assetsP1.crypto), mode==="Couple"?Math.round(r.assetsP2.tfsa):null, mode==="Couple"?Math.round(r.assetsP2.rrsp):null, mode==="Couple"?Math.round(r.assetsP2.nreg):null, mode==="Couple"?Math.round(r.assetsP2.cash):null, mode==="Couple"?Math.round(r.assetsP2.crypto):null, Math.round(r.liquidNW), Math.round(r.homeValue-r.mortgage), Math.round(r.debugNW)].filter(x=>x!==null).join(","));
    ...
    addWindfall() { this.state.windfalls.push({ name: "New Inheritance", amount: 0, freq: 'one', owner: 'p1', taxable: false, start: new Date().toISOString().slice(0, 7), end: '' }); this.renderWindfalls(); this.run(); }
    removeWindfall(index) { this.showConfirm("Remove item?", () => { this.state.windfalls.splice(index, 1); this.renderWindfalls(); this.run(); }); }
    ...
    updatePropPayoffDisplay(idx) {
        ...
        const r = (p.rate/100)/12; if(p.payment <= p.mortgage*r) return el.innerHTML = `<span class="text-danger fw-bold">Payment too low</span>`;
        const nMonths = -Math.log(1 - (r*p.mortgage)/p.payment) / Math.log(1+r);
        if(isFinite(nMonths)) el.innerHTML = `<span class="text-success fw-bold">Payoff: ${Math.floor(nMonths/12)}y ${Math.round(nMonths%12)}m</span>`;
    }
    ...
    renderExpenseRows() {
        ...
        th.innerHTML = this.state.expenseMode==='Simple' ? `<th class="text-uppercase small ps-3 ${hc}" style="width: 40%;">Category / Item</th><th class="text-uppercase small ${hc}" style="width: 30%;">Current</th><th class="text-uppercase small ${hc}" style="width: 30%;">Retirement</th>` : `<th class="text-uppercase small ps-3 ${hc}" style="width: 20%;">Item</th><th class="text-uppercase small ${hc}" style="width: 16%;">Current</th><th class="text-uppercase small ${hc}" style="width: 16%;">Trans</th><th class="text-uppercase small ${hc}" style="width: 16%;">Go-Go</th><th class="text-uppercase small ${hc}" style="width: 16%;">Slow-Go</th><th class="text-uppercase small ${hc}" style="width: 16%;">No-Go</th>`;
    ...
    renderStrategy() {
        ...
        const d = document.getElementById('strat-decumulation'); d.innerHTML = `<div class="card bg-black border-secondary mb-3 strategy-opt-box"><div class="card-header border-secondary text-uppercase small fw-bold text-muted bg-dark bg-opacity-50"><i class="bi bi-stars text-warning me-2"></i>Optimization Strategies</div><div class="card-body p-3"><div class="form-check form-switch"><input class="form-check-input live-calc" type="checkbox" role="switch" id="strat_rrsp_topup" ${this.state.inputs['strat_rrsp_topup']?'checked':''}><label class="form-check-label text-white small fw-bold" for="strat_rrsp_topup">RRSP Low-Income Top-Up<div class="text-muted fw-normal mt-1" style="font-size:0.75rem; line-height:1.2;">Withdraws RRSP to fill the lowest tax bracket (~$55k) in years with low income.</div></label></div></div></div><h6 class="text-white small fw-bold mb-2 text-uppercase">Withdrawal Order (Drag to Reorder)</h6>`;
    ...
    renderTaxDetails(pfx, g, d) {
        ...
        c.innerHTML = g>0 ? `<div class="d-flex justify-content-between border-bottom border-secondary pb-1 mb-1"><span class="text-muted">Federal</span> <span class="text-white">($${Math.round(d.fed).toLocaleString()}) ${((d.fed/g)*100).toFixed(1)}%</span></div><div class="d-flex justify-content-between border-bottom border-secondary pb-1 mb-1"><span class="text-muted">Provincial</span> <span class="text-white">($${Math.round(d.prov).toLocaleString()}) ${((d.prov/g)*100).toFixed(1)}%</span></div><div class="d-flex justify-content-between border-bottom border-secondary pb-1 mb-1"><span class="text-muted">CPP/EI</span> <span class="text-white">($${Math.round(d.cpp_ei).toLocaleString()})</span></div><div class="d-flex justify-content-between mt-2"><span class="text-warning fw-bold">Total Tax</span> <span class="text-warning fw-bold">($${Math.round(d.totalTax).toLocaleString()})</span></div><div class="d-flex justify-content-between"><span class="text-muted">Marginal Rate</span> <span class="text-white">${(d.margRate*100).toFixed(2)}%</span></div><div class="d-flex justify-content-between mt-2 pt-2 border-top border-white"><span class="text-success fw-bold">After-Tax</span> <span class="text-success fw-bold">$${Math.round(g-d.totalTax).toLocaleString()}</span></div>` : `<span class="text-muted text-center d-block small">No Income Entered</span>`;
    }
    ...
    loadStateToDOM(d) {
        this.state.inputs = {...d.inputs}; this.state.strategies = {...d.strategies};
        ...
        if(d.expensesData) this.expensesByCategory = d.expensesData; this.renderExpenseRows();
        if(d.properties) this.state.properties = d.properties; this.renderProperties();
        if(d.windfalls) this.state.windfalls = d.windfalls; this.renderWindfalls();
        if(d.additionalIncome) this.state.additionalIncome = d.additionalIncome; this.renderAdditionalIncome();
    ...
    getCurrentSnapshot() { const s = { inputs: {...this.state.inputs}, strategies: {...this.state.strategies}, debt: [], properties: JSON.parse(JSON.stringify(this.state.properties)), expensesData: JSON.parse(JSON.stringify(this.expensesByCategory)), windfalls: JSON.parse(JSON.stringify(this.state.windfalls)), additionalIncome: JSON.parse(JSON.stringify(this.state.additionalIncome)) }; document.querySelectorAll('.debt-amount').forEach(el=>s.debt.push(el.value)); return s; }
}
