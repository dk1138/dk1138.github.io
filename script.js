/**
 * Retirement Planner Pro - Logic v10.45 (Iterative Deficit & Real Estate UI Fix)
 * * Changelog:
 * - v10.45: FIXED: "Surplus" calculation is now Iterative. The system runs up to 5 passes per year to withdraw additional funds to cover taxes generated by previous withdrawals, ensuring Net Cash is close to $0.
 * - v10.45: FIXED: Real Estate payment input is now debounced. Changing the payment updates the "Payoff Date" label smoothly without lag.
 * - v10.43: FIXED: Asset list Crypto line item & Independent spouse withdrawal order.
 */
class RetirementPlanner {
    constructor() {
        this.APP_VERSION = "10.45";
        this.state = {
            inputs: {},
            debt: [],
        ...
        for (let i=0; i<=yearsToRun; i++) {
            ...
            if (surplus > 0) {
                ...
                for(let pass=0; pass<5; pass++) {
                    // Current Net Cash = Total Inflow so far - Expenses - Taxes
                    // We need to withdraw enough to make this >= 0
                    
                    let currentWithdrawals = Object.values(flowLog.withdrawals).reduce((a,b)=>a+b,0);
                    let currentNetCash = (totalNetIncome + currentWithdrawals) - totalOutflows; 
                    // Note: totalNetIncome calculated initially includes tax1.totalTax based on initial income.
                    // But we re-calc tax below. We should base deficit on dynamic values.
                    ...
                    // Deficit = Outflows - (Net Income + Withdrawals taken so far)
                    // Note: Withdrawals taken so far are already inside the assets (reduced) and logs. 
                    // But they are NOT inside 'taxableIncome' variables yet? 
                    // Wait, handleDeficit UPDATES taxableIncome1/2 via callback or we must do it manually?
                    // In previous code, handleDeficit took a callback "onTaxIncrease" but didn't update taxable income.
                    // We need handleDeficit to update the taxableIncome variables for the NEXT pass.
                    
                    let currentDeficit = totalOutflows - (dynTotalNet + 0); // Withdrawals are effectively spent. 
                    // Actually, logic is: We need X cash. We have Y net income. Gap is Z.
                    // Withdraw Z. 
                    // Next pass: Income went up by Z (gross). Tax went up. Net Income changed.
                    // We need to verify if (Net Income + Z_net) == Outflows.
                    // Simpler: Just track cash bucket.
                    
                    // Cash Bucket = (Net Income from all sources) - Expenses.
                    // Since withdrawals are added to income, they increase Net Income.
            ...
            if(!onlyCalcNW) {
                ...
                // However, for the Sankey/Grid, we usually separate Net Income from Withdrawals. 
                // To adapt to the existing display logic:
                // Grid expects: Income + Withdrawals - Expenses - Tax = Surplus
                // Currently 'taxableIncome' includes withdrawals.
                // We need to revert 'incomeP1' to just base income for display, or keep it.
                // The existing code uses 'inflows.p1.gross' etc which are base incomes.
                // The withdrawals are in 'flowLog'.
                // So we can reconstruct the view.
    ...
    handleDeficit(amount, p1, p2, curInc1, curInc2, alive1, alive2, log, breakdown, taxBrackets, onWithdrawal, age1, age2) {
        ...
        const wd = (p, t, a, pfx, mRate) => { 
            ...
            // For NREG, we only get cash from the full withdrawal. The tax is calculated later.
            // But we need to ensure we take enough principal+gain to cover the deficit 'a'.
            // Actually, if we take 'a' from NREG, we get 'a' cash. But we create a tax liability.
            // The outer loop handles the "second pass" for that tax liability.
            // So for NREG, just take 'a'.
            ...
            if (isTaxable) {
                 ...
                 // For Non-Reg, calculate gain
                 // If p[t] is now 0, we took everything. We need the ratio from BEFORE the reduction.
                 // But we can't easily get it. 
                 // Logic fix: pass ratio or calc before reduction.
                 // Simplified: Assume average ACB ratio is constant for this transaction
                 // This is tricky. Let's rely on the outer loop re-calc for precision tax.
                 // For the callback (updating income for this loop), we need the taxable portion.
                 // This is handled better in the calling function via re-calc.
                 // Here we just notify gross amount withdrawn.
                 // Wait, Non-Reg withdrawal increases CASH but only PARTIAL income.
                 // The callback expects (pfx, taxAmt, GROSS_INCOME_ADDITION).
                 // We don't calculate taxAmt here anymore (handled in outer loop).
                 // We need to calculate TAXABLE portion of this withdrawal.
                 // let gain = tk * (1 - (p.acb / (p.nreg + tk))); // Approx
                 // if(onWithdrawal) onWithdrawal(pfx, 0, gain * 0.5);
                 // Actually, let's just let the outer loop recalc income based on assets.
                 // But we need to update runInc for the inner logic.
                 // Let's defer NREG income updates to the outer loop since they are complex.
            }
